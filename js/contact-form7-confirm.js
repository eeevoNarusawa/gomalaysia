...
jQuery(document).ready(function(){
	...
	var option = {
		pages : [
			{
				'path' : ['/inquiry/','/malaysia-properties-inquiry/','/malaysia-stay-inquiry/'],
				'button' : {
					...
					'rewrite' : '<input class="button-rewrite" type="button" value="入力内容修正" />'	//	html
				}
			}
		],
		validates : {
	...
	.each(function(){
		...
		jQuery(this).find('.addplus-contact').show();
		...
		.click(function(){
			...
			form.find('.addplus-contact').show();
		...
		.click(function(){
			...
			if (error.children().length > 0){
				...
				form.find('.addplus-contact').hide();
			}
			jQuery('html,body').animate({ scrollTop: form.offset().top - 30}, 'slow', null);
			return false;
		});
